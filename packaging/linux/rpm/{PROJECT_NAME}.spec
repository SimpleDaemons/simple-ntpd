...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-ntpd - A lightweight and secure daemon
License:        Apache-2.0
...
%description
simple-ntpd is a modern, secure implementation of the NTP protocol.
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id ntpddev &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" ntpddev
    fi
fi
