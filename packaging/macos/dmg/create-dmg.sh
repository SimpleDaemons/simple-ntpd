#!/bin/bash
# Create DMG installer for simple-ntpd
...
VERSION="${1:-1.0.0}"
DMG_NAME="$simple-ntpd-${VERSION}"
DMG_TEMP="dmg-temp"
DMG_FILE="${DMG_NAME}.dmg"
APP_NAME="$simple-ntpd.app"
PKG_NAME="$simple-ntpd.pkg"
...
if [ -f "../assets/icons/$simple-ntpd.icns" ]; then
    # Set custom icon (requires additional tools)
    echo "Custom icon available at ../assets/icons/$simple-ntpd.icns"
fi
