...
struct NtpTimestamp {
     ...
     */
    static NtpTimestamp fromSystemTime(const std::chrono::system_clock::time_point& time);
...
private:
     ...
     */
    std::chrono::milliseconds calculateRoundTripDelay(
     ...
     */
    std::chrono::milliseconds calculateOffset(
        const NtpTimestamp& t1,
        const NtpTimestamp& t2,
        const NtpTimestamp& t3,
        const NtpTimestamp& t4) const;
