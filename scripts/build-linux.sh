#!/bin/bash
# Generic Linux build script for simple-ntpd
...
build_project() {
    print_status "Building simple-ntpd..."
...
install_project() {
    if [ "$INSTALL" = "true" ]; then
        print_status "Installing simple-ntpd..."
        cd "$BUILD_DIR"
        
        sudo make install
        
        # Test installation
        if command_exists simple-ntpd; then
...
create_service() {
    if [ "$SERVICE" = "true" ]; then
        ...
        sudo tee /etc/systemd/system/simple-ntpd.service > /dev/null << EOF
[Unit]
Description=Simple NTP Daemon - A lightweight and secure NTP server
...
User=simple-ntpd
Group=simple-ntpd
ExecStart=/usr/local/bin/simple-ntpd --daemon
Restart=always
RestartSec=5
...
EOF
        ...
        sudo useradd --system --no-create-home --shell /bin/false simple-ntpd 2>/dev/null || true
        ...
        print_status "Use 'sudo systemctl start simple-ntpd' to start the service"
    fi
}
...
show_usage() {
    cat << EOF
simple-ntpd - Linux Build Script
...
# Main execution
main() {
    print_status "Starting simple-ntpd build process..."
    ...
    print_success "simple-ntpd build process completed successfully!"
}
