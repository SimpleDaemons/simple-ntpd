...
# simple-ntpd macOS Build Script
# This script builds the simple-ntpd application for macOS
...
create_static_package() {
    if [ "$ENABLE_STATIC_LINKING" = "ON" ]; then
        ...
        STATIC_DIR="$DIST_DIR/simple-ntpd-$VERSION-static-macos"
        mkdir -p "$STATIC_DIR"
        
        # Copy binary and files
        cp "$BUILD_DIR/simple-ntpd" "$STATIC_DIR/"
        ...
        # Create tarball
        cd "$DIST_DIR"
        tar -czf "simple-ntpd-$VERSION-static-macos.tar.gz" "simple-ntpd-$VERSION-static-macos"
        rm -rf "simple-ntpd-$VERSION-static-macos"
        
        print_success "Static binary package created: simple-ntpd-$VERSION-static-macos.tar.gz"
    fi
}
