...
:build_project
call :print_status "Building simple-ntpd..."
...
:install_project
if "%INSTALL%"=="true" (
    call :print_status "Installing simple-ntpd..."
    cd /d "%BUILD_DIR%"
...
if "%ENABLE_STATIC_LINKING%"=="ON" (
    ...
    set "STATIC_DIR=%DIST_DIR%\simple-ntpd-0.1.0-static-windows"
    if not exist "%STATIC_DIR%" mkdir "%STATIC_DIR%"
    
    REM Copy binary and files
    copy "%BUILD_DIR%\simple-ntpd.exe" "%STATIC_DIR%\"
    ...
    cd /d "%DIST_DIR%"
    powershell -Command "Compress-Archive -Path 'simple-ntpd-0.1.0-static-windows' -DestinationPath 'simple-ntpd-0.1.0-static-windows.zip' -Force"
    rmdir /s /q "simple-ntpd-0.1.0-static-windows"
    
    call :print_success "Static binary package created: simple-ntpd-0.1.0-static-windows.zip"
)
goto :eof

REM Function to show usage
:show_usage
echo simple-ntpd - Windows Build Script
echo.
...
REM Main execution
:main
call :print_status "Starting simple-ntpd build process..."
...
call :print_success "simple-ntpd build process completed successfully!"
goto :end
