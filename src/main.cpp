/**
 * @file main.cpp
 * @brief Simple NTP Daemon main entry point
...
void signalHandler(int signal) {
  ...
  g_logger->info("Received signal " + std::to_string(signal) +
 ...
 */
void printUsage() {
  std::cout << "\nUsage: simple-ntpd [OPTIONS] [COMMAND] [ARGS...]"
            << std::endl;
  std::cout << "\nOptions:" << std::endl;
  std::cout << "  --help, -h           Show this help message" << std::endl;
  std::cout << "  --version, -v        Show version information" << std::endl;
  std::cout << "  --config, -c FILE    Use specified configuration file"
            << std::endl;
  ...
  std::cout << "\nExamples:" << std::endl;
  std::cout << "  simple-ntpd start --config /etc/simple-ntpd/config.conf"
            << std::endl;
  std::cout << "  simple-ntpd start --listen 0.0.0.0 --port 123 --stratum 2"
            << std::endl;
    ...
    if (arg == "--help" || arg == "-h") {
      ...
      if (i + 1 < argc) {
        ...
        if (!config->loadFromFile(argv[++i])) {
          std::cerr << "Error: Failed to load configuration file: " << argv[i]
                    << std::endl;
          return false;
        }
      } else {
      ...
      if (i + 1 < argc) {
        ...
        std::cerr << "Error: --reference-id requires an identifier"
                  << std::endl;
        return false;
      }
...
int main(int argc, char *argv[]) {
  try {
    ...
    g_server = std::make_shared<NtpServer>(
        config,
        std::shared_ptr<Logger>(&Logger::getInstance(), [](Logger *) {}));
