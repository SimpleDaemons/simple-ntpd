...
NtpPacket NtpPacket::createServerResponse(const NtpPacket& client_packet,
    ...
    packet.receive_ts = NtpTimestamp::fromSystemTime(std::chrono::system_clock::now());
    packet.transmit_ts = NtpTimestamp::fromSystemTime(std::chrono::system_clock::now());
    
    // Set reference timestamp and ID
    packet.reference_ts = NtpTimestamp::fromSystemTime(std::chrono::system_clock::now());
...
std::string NtpPacket::getTypeDescription() const {
    ...
    switch (static_cast<NtpMode>(mode)) {
        case NtpMode::CLIENT: ss << "Client"; break;
        case NtpMode::SERVER: ss << "Server"; break;
        case NtpMode::BROADCAST: ss << "Broadcast"; break;
...
bool NtpPacketHandler::parsePacket(const std::vector<uint8_t>& data, NtpPacket& packet) {
    return packet.parseFromData(data);
}

NtpPacket NtpPacketHandler::createResponsePacket(const NtpPacket& request_packet,
                                                const std::shared_ptr<NtpConfig>& config) {
    // Create server response with default values
    return NtpPacket::createServerResponse(request_packet, 
                                         NtpStratum::SECONDARY_REFERENCE, 
                                         "LOCL");
}
...
template<typename T>
T NtpPacketHandler::hton(T value) const {
...
template<typename T>
T NtpPacketHandler::ntoh(T value) const {
    if constexpr (sizeof(T) == 2) {
        return ntohs(value);
    } else if constexpr (sizeof(T) == 4) {
        return ntohl(value);
